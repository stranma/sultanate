...
wg_conf_path = f"/opt/sultanate/provinces/{province_id}/wg0.conf"
...
subprocess.run([
    ...
    "--name", sidecar_name,
    "--cap-add", "NET_ADMIN",
    "--volume", f"{wg_conf_path}:/etc/wireguard/wg0.conf:ro",
...
# Province IP is the WireGuard peer address assigned during wg-client creation
# (stored in the wg0.conf generated by Vizier)
container_ip = province_wg_address  # e.g., "10.13.13.2"

# Update Divan with WireGuard IP
...
Vizier attaches to this network. Province containers do NOT attach directly -- instead, each province gets a `wg-client` sidecar container, and the province shares the sidecar's network namespace (`--network-mode container:wg-client-{id}`). The wg-client sidecar establishes a WireGuard tunnel to Janissary and uses iptables to route all traffic through mitmproxy. This is transparent to the province -- no proxy configuration needed.
...
Province containers do not need proxy environment variables. All traffic is transparently routed through Janissary via WireGuard.
...
**Inspect IP:**
Province IP is the WireGuard peer address from the generated `wg0.conf`, not a Docker network IP. It's stored in Divan at creation time.
...
- Janissary HTTP API is included so Vizier can appeal blocks on its own traffic.
...
Vizier is on `sultanate-internal`. All egress (including Telegram API calls from Hermes) routes through Janissary via WireGuard transparent proxy. The Docker socket is mounted so Vizier can create/manage province containers on the host.
